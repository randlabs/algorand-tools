 ...
 *
 * @param {Object} tx Transaction from Algorand
...
async function addSignature(tx, mnemonic_or_signer, multisig_threshold /*= undefined*/, multisig_addresses /*= undefined*/) {
	...
	let sender = addresses.encode(tx.txn.snd);
	...
	if (typeof mnemonic_or_signer === 'string') {
		...
		let algo_tx = algosdk_transaction.Transaction.from_obj_for_encoding(tx.txn);
		...
		let signature = await mnemonic_or_signer.signRawTransaction(algosdk.encodeObj(tx.txn));
		signer_info = {
			address,
			signature
		};
	...
	if (!multisig_threshold) {
		if (utils.isObject(tx.msig)) {
			if (utils.isInteger(tx.msig.thr)) {
				multisig_threshold = tx.msig.thr;
			}
		}
	}
	...
	if (!multisig_threshold) {
		...
		if (utils.isObject(tx.msig)) {
			...
			if (typeof tx.msig.v === 'number') {
				new_msig.v = tx.msig.v;
			}
			//check threshold
			if (typeof tx.msig.thr === 'number' && tx.msig.thr != multisig_threshold) {
				...
				if (!multisig_addresses) {
					...
					for (let idx = 0; idx < tx.msig.subsig.length; idx++) {
						if (typeof tx.msig.subsig[idx].pk != 'undefined') {
							let address = addresses.encode(tx.msig.subsig[idx].pk);

							multisig_addresses.push(address);

							if (typeof tx.msig.subsig[idx].s != 'undefined') {
								prev_signatures.set(address, tx.msig.subsig[idx].s);
							}
						}
					}
 ...
 *
 * @param {Object} tx Algorand Transaction object
 */
 ...
 *
 * @param {Object} tx Algorand Transaction format
 * @param {Number} multisig_threshold Required signatures
 * @param {Array} multisig_addresses Array of public keys
 */

function addSignatureTemplate(tx, multisig_threshold /*= undefined*/, multisig_addresses /*= undefined*/) {
	if (!(utils.isObject(tx) && utils.isObject(tx.txn))) {
		throw new Error('Invalid tx (tx-sign).');
	}
	...
	if (addresses.encode(mergedAccountPk) !== addresses.encode(tx.txn.snd)) {
 ...
 *
 * @param {Object} tx Algorand Transaction
...
function getMultiSignatureSigners(tx) {
	...
	if (tx.msig) {
		if (tx.msig.subsig) {
			for (let idx = 0; idx < tx.msig.subsig.length; idx++) {
				if (typeof tx.msig.subsig[idx].pk != 'undefined') {
					_addresses.push(addresses.encode(tx.msig.subsig[idx].pk));
				}
			}
		}
	}
...
function verifySignature(tx, signature, address) {
	...
	let algo_tx;
	try {
		algo_tx = algosdk_transaction.Transaction.from_obj_for_encoding(tx.txn);
	}
	catch (err) {
		return false;
	}
