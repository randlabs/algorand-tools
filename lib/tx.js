...
function getInfo(tx) {
	...
	if (typeof tx.txn.type === 'string') {
		...
		if (typeof tx.txn.note != 'undefined') {
			params.note = Buffer.from(tx.txn.note).toString('base64');
		}
		if (typeof tx.txn.lx != 'undefined') {
			params.lease = Buffer.from(tx.txn.lx).toString('base64');
		}
		if (typeof tx.txn.gen != 'undefined') {
			params.genesis_id = tx.txn.gen;
		}
		if (typeof tx.txn.gh != 'undefined') {
			params.genesis_hash = Buffer.from(tx.txn.gh).toString('base64');
		}
		if (typeof tx.txn.group != 'undefined') {
			params.group = Buffer.from(tx.txn.gh).toString('base64');
		}
		...
		if (tx.txn.type === 'pay') {
			...
			if (typeof tx.txn.votekey != 'undefined') {
				params.vote_key = Buffer.from(tx.txn.votekey).toString('base64');
			}
			if (typeof tx.txn.selkey != 'undefined') {
				params.selection_key = Buffer.from(tx.txn.selkey).toString('base64');
			}
			...
			if (typeof tx.txn.apar != 'undefined') {
				if (typeof tx.txn.apar.m != 'undefined') {
					params.asset_manager = addresses.encode(tx.txn.apar.m);
				}
				if (typeof tx.txn.apar.r != 'undefined') {
					params.asset_reserve = addresses.encode(tx.txn.apar.r);
				}
				if (typeof tx.txn.apar.f != 'undefined') {
					params.asset_freeze = addresses.encode(tx.txn.apar.f);
				}
				if (typeof tx.txn.apar.c != 'undefined') {
					params.asset_clawback = addresses.encode(tx.txn.apar.c);
				}
				if (typeof tx.txn.apar.am != 'undefined') {
					params.asset_metahash_data = Buffer.from(tx.txn.apar.m).toString('base64');
				}
