...
async function createKeyregTransaction(options) {
	...
	options.vote_key = validateBuffer(options.vote_key, false, 'Invalid vote key.');
	options.selection_key = validateBuffer(options.selection_key, false, 'Invalid selection key.');
	options.vote_first = validateUint(options.vote_first, false, 'Invalid vote first.');
	options.vote_last = validateUint(options.vote_last, false, 'Invalid vote last.');
	options.vote_key_dilution = validateUint(options.vote_key_dilution, false, 'Invalid vote key dilution.');

	let tx_params = buildTxParamsBase(options, 'keyreg');
	if (options.vote_key !== null) {
		tx_params.voteKey = options.vote_key;
	}
	if (options.selection_key !== null) {
		tx_params.selectionKey = options.selection_key;
	}
	if (options.vote_first !== null) {
		tx_params.voteFirst = options.vote_first;
	}
	if (options.vote_last !== null) {
		tx_params.voteLast = options.vote_last;
	}
	if (options.vote_key_dilution !== null) {
		tx_params.voteKeyDilution = options.vote_key_dilution;
	}
...
async function createAssetCreationTransaction(options) {
	...
	options.asset_metahash_data = validateBuffer(options.asset_metahash_data, true, 'Invalid asset metahash data.');
	...
	options.asset_total = validateBuffer(options.asset_total, false, 'Invalid asset total.');
...
async function createAssetTransferTransaction(options) {
	...
	tx_params.assetIndex = options.asset_index;
	tx_params.amount = options.freeze_account;
	if (options.close !== null) {
		tx_params.closeRemainderTo = options.close;
	}
	...
	if (typeof tx.txn.type === 'string') {
		...
		if (tx.txn.type === 'pay') {
			...
			if (typeof tx.txn.apar != 'undefined') {
				...
				params.asset_total = (typeof tx.txn.apar.t != 'undefined') ? tx.txn.apar.t : 0;
				params.asset_name = (typeof tx.txn.apar.an == 'string') ? tx.txn.apar.an : '';
				params.asset_unit_name = (typeof tx.txn.apar.un == 'string') ? tx.txn.apar.un : '';
				if (typeof tx.txn.apar.au == 'string') {
					params.asset_url = tx.txn.apar.un;
				}
				params.asset_default_frozen = (typeof tx.txn.apar.df != 'undefined') ? Boolean(tx.txn.apar.df) : false;
			}
			else {
				params.asset_total = 0;
				params.asset_name = '';
				params.asset_unit_name = '';
				params.asset_default_frozen = false;
			}
		}
...
async function validateCommonTxOptions(options) {
	...
	options.genesis_hash = validateBuffer(options.genesis_hash, true, 'Invalid genesis hash.');

	if (typeof options.genesis_id !== 'undefined') {
		if (typeof options.genesis_id !== 'string' || options.genesis_id.length == 0) {
			throw new Error('Invalid genesis id.');
		}
	}

	options.note = validateBuffer(options.note, true, 'Invalid note.');
	if (!options.note) {
		options.note = new Uint8Array(0);
	}

	options.lease = validateBuffer(options.lease, true, 'Invalid lease.');
	if (!options.lease) {
	...
	options.group = validateBuffer(options.group, true, 'Invalid group.');
...
function validateBuffer(value, canBeNull, invalidMessage) {
	if (typeof value === 'undefined' || value === null) {
		if (!canBeNull) {
			throw new Error(invalidMessage);
		}
